...
describe("LinkControl test suite", () => {
  ...
  it("should have a div when rendered", () => {
    expect(mount(<LinkControl onChange={() => {}} editorState={editorState} config={defaultToolbar.link} translations={localeTranslations.en} modalHandler={new ModalHandler()} />).html().startsWith("<div")).to.equal(true);
  });

  it("should have 2 child elements by default", () => {
    const control = mount(<LinkControl onChange={() => {}} editorState={editorState} config={defaultToolbar.link} translations={localeTranslations.en} modalHandler={new ModalHandler()} />);
  ...
  it("should convert links starting with www to start with http://", () => {
    const onChange = spy();
    const control = mount(<LinkControl config={defaultToolbar.link} onChange={onChange} editorState={editorState} translations={localeTranslations.en} modalHandler={new ModalHandler()} />);
