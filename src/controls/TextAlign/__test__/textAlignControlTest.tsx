...
describe('TextAlignControl test suite', () => {
  ...
  it('should have a div when rendered', () => {
    expect(mount(<TextAlignControl onChange={() => {}} editorState={editorState} config={defaultToolbar.textAlign} translations={localeTranslations.en} modalHandler={new ModalHandler()} />).html().startsWith('<div')).to.equal(true);
  });

  it('should have 4 child elements by default', () => {
    const control = mount(<TextAlignControl onChange={() => {}} editorState={editorState} config={defaultToolbar.textAlign} translations={localeTranslations.en} modalHandler={new ModalHandler()} />);
  ...
  it('should have 5 child options of dropdown child elements if inDropdown is true', () => {
    const control = mount(<TextAlignControl onChange={() => {}} editorState={editorState} config={{ ...defaultToolbar.textAlign, inDropdown: true }} translations={localeTranslations.en} modalHandler={new ModalHandler()} />);
  ...
  it('should execute onChange when any of first any child elements is clicked', () => {
    const onChange = spy();
    const control = mount(<TextAlignControl onChange={onChange} editorState={editorState} config={defaultToolbar.textAlign} translations={localeTranslations.en} modalHandler={new ModalHandler()} />);
