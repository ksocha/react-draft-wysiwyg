...
describe('InlineControl test suite', () => {
  ...
  it('should have a div when rendered', () => {
    expect(mount(<InlineControl onChange={() => {}} editorState={editorState} config={defaultToolbar.inline} translations={localeTranslations.en} modalHandler={new ModalHandler()} />).html().startsWith('<div')).to.equal(true);
  });

  it('should have 5 child elements by default', () => {
    const control = mount(<InlineControl onChange={() => {}} editorState={editorState} config={defaultToolbar.inline} translations={localeTranslations.en} modalHandler={new ModalHandler()} />);
    expect(control.find(Option).length).to.equal(7);
  });

  it('should have 1 child elements if inDropdown is true', () => {
    const control = mount(<InlineControl onChange={() => {}} editorState={editorState} config={{ ...defaultToolbar.inline, inDropdown: true }} translations={localeTranslations.en} modalHandler={new ModalHandler()} />);
  ...
  it('should execute onChange when child elements are clicked', () => {
    const onChange = spy();
    const control = mount(<InlineControl onChange={onChange} editorState={editorState} config={defaultToolbar.inline} translations={localeTranslations.en} modalHandler={new ModalHandler()} />);
  ...
  it('should have false value for all rich styles in state by default', () => {
    const control = shallow(<InlineControl onChange={() => {}} editorState={editorState} config={defaultToolbar.inline} modalHandler={new ModalHandler()} />);
